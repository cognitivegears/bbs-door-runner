...
get readOnly() {
    ...
    readSectors(i, dest, cb) {
        if (dest.length % this.sectorSize) throw Error("Unexpected buffer length!");
        fs.read(this._fd, dest, 0, dest.length, this._partitionOffsetBytes + (i * this.sectorSize), function (e, n, d) {
            cb(e, d);
        });
    }
    ...
    writeSectors(i, data, cb) {
        const fs = require('fs');
        if (data.length % this.sectorSize) throw Error("Unexpected buffer length!");
        fs.write(this._fd, data, 0, data.length, this._partitionOffsetBytes + (i * this.sectorSize), function (e) {
            cb(e);
        });
    }
